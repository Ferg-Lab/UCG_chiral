...
void Atom::peratom_create()
{
  ...
  add_peratom("chirality",&chirality,DOUBLE,0); // [SD]
...
void *Atom::extract(const char *name)
{
  ...
  // UCG package
  if (strcmp(name,"chirality") == 0) return (void *) chirality; // [SD]
...
int Atom::extract_datatype(const char *name)
{
  ...
  // UCG package
  if (strcmp(name,"chirality") == 0) return LAMMPS_DOUBLE; // [SD]
...
int Atom::extract_size(const char *name, int type)
{
  ...
  if ((datatype == LAMMPS_DOUBLE_2D) || (datatype == LAMMPS_INT_2D)) {
    ...
    // UCG package
    if (strcmp(name,"chirality") == 0) return nall; // [SD]
