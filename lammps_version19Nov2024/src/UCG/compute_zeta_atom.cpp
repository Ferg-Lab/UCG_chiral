...
ComputeZetaAtom::ComputeZetaAtom(LAMMPS *lmp, int narg, char **arg) :
...
ComputeZetaAtom::~ComputeZetaAtom()
{	
  memory->destroy(zeta);
}
...
void ComputeZetaAtom::init()
{
}

/* ---------------------------------------------------------------------- */

void ComputeZetaAtom::compute_peratom()
{	
  int i1,i2,i3,i4;
  int i,m,n,type;
  ...
  if (atom->nmax > nmax) {
    memory->destroy(zeta);
    nmax = atom->nmax;
    memory->create(zeta,nmax,"zeta/atom:zeta");
    vector_atom = zeta;
  }
...
int ComputeZetaAtom::pack_forward_comm(int n, int *list, double *buf, int pbc_flag,
    int *pbc) {
  int i, j, m;
...
void ComputeZetaAtom::unpack_forward_comm(int n, int first, double *buf) {
  int i, m, last;
...
int ComputeZetaAtom::pack_reverse_comm(int n, int first, double *buf)
{
  int i, m,last;
...
void ComputeZetaAtom::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
double ComputeZetaAtom::memory_usage()
{
