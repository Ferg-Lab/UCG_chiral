...
PairTableUCGDNA::PairTableUCGDNA(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = nullptr;
...
PairTableUCGDNA::~PairTableUCGDNA()
{
  ...
  memory->destroy(pair_distances); // SD
...
void PairTableUCGDNA::threshold_prob_and_partial_from_cv(int type, double cv, double &prob, double &partial) 
{
  if (type == 1){
...
int PairTableUCGDNA::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairTableUCGDNA::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
int PairTableUCGDNA::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m,jsubstate;
...
void PairTableUCGDNA::unpack_forward_comm(int n, int first, double *buf)
{
...
// Compute local scaled proximity function (tanh functions)
double PairTableUCGDNA::compute_proximity_function(int type, double distance) // SD
{
  //double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
  return distance; //neg_chirality * 0.5 * (1.0 - tanh_factor); 
}

double PairTableUCGDNA::compute_proximity_function_der(int type, double distance) // SD
{
  //double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
  return 1.0; //neg_chirality * 0.5 * (1.0 - tanh_factor * tanh_factor) / (0.1 * threshold_radii[type]);
}

/* ---------------------------------------------------------------------- */

void PairTableUCGDNA::compute(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype,itable;
  int itype_actual, jtype_actual;
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair;
  double rsq,factor_lj,fraction,value,a,b;
  double distance,cv_force; 
  ...
  if (atom->nmax > nmax) {
    ...
    memory->create(pair_distances, nmax, "pair:pair_distances"); // SD
  }
  ...
  for (ii = 0; ii < inum; ii++) {
      ...
      for (isubstate = 0; isubstate < n_states_per_type[itype_actual]; isubstate++) {
        ...
        for (jj = 0; jj < jnum; jj++) {
          ...
          if (rsq < cutsq[itype][jtype]) {
            ...
            if (isubstate == 0) {
              //printf("LOOP 1 i %d isubstate %d j %d distance %f\n", tag[i], isubstate, tag[j], distance);
              pair_distances[i] = compute_proximity_function(itype_actual, distance);
            }

            #ifdef DEBUG_PROBABILITIES
              // SD: log number densities for debugging.
              fprintf(densityoutfile, BIGINT_FORMAT " Step: %d tag-i: %d tag-j: %d itype: %d jtype: %d itype actual: %d isubstate:%d pf: %lg ee: %lg \n", update->ntimestep, tag[i], tag[j], itype, jtype, itype_actual, isubstate, compute_proximity_function(itype_actual, distance), pair_distances[i]);
            #endif
          }
        }

        if (isubstate == 0) {
          threshold_prob_and_partial_from_cv(itype_actual, pair_distances[i], substate_probability[i][isubstate], substate_probability_partial[i][isubstate]);// [SD]
        }
        else {
        ...
        //printf("i: %d, isubstate:%d, ee: %f, p:%f pd: %f\n", tag[i], isubstate, pair_distances[i], substate_probability[i][isubstate],  substate_probability_partial[i][isubstate]);
      }
    } else {
    ...
    for (j = 0; j < max_states_per_type; j++) {
      //printf("i %d, tagi %d j %d, substate_probability[i][j] %f partial[i][j] %f \n", i, tag[i], j, substate_probability[i][j], substate_probability_partial[i][j]);
  ...
  // Communicate pair_distances and state probabilities with neighboring procs
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    /* SD: No entropy and no chemical potentials needed anyway.
    if (n_states_per_type[itype_actual] > 1) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        for (isubstate = 0; isubstate < n_states_per_type[itype_actual]; isubstate++) {
          ...
          for (jsubstate = 0; jsubstate < n_states_per_type[jtype_actual]; jsubstate++) {
            ...
            //printf("\n \n i: %d j: %d isub: %d jsub: %d FPAIR: %f alphaprob %f betaprob %f delz %f \n \n", tag[i], tag[j], isubstate, jsubstate, fpair, alphaprob, betaprob, delz);

            fpair = fpair * alphaprob * betaprob; // [SD]: p(s_i | R) x p(s_j | R) x ∇U_si_sj(R_ij)

            //printf("\n \n AFTER i: %d j: %d isub: %d jsub: %d FPAIR: %f alphaprob %f betaprob %f delz %f \n \n", tag[i], tag[j], isubstate, jsubstate, fpair, alphaprob, betaprob, delz);
            ...
            */
            energy_lj += evdwl * alphaprob * betaprob; // halved to prevent double counting energy contribution.
            pair_force += fpair;

            /* DEBUGGING
            if (j < nlocal){
              printf("LOCAL atoms i %d j %d isub %d jsub %d d %f fpair %f ap %f bp %f fx %f fy %f fz %f e %f\n", tag[i], tag[j], isubstate, jsubstate, distance, fpair, alphaprob, betaprob, f[i][0], f[i][1], f[i][2], energy_lj);
            }
            else {
              printf("GHOST atoms i %d j %d isub %d jsub %d d %f fpair %f ap %f bp %f fx %f fy %f fz %f e %f\n", tag[i], tag[j], isubstate, jsubstate, distance, fpair, alphaprob, betaprob, f[i][0], f[i][1], f[i][2], energy_lj);
            }
            */
            ...
            if (n_states_per_type[itype_actual] > 1) {
              /* SD: NOT SURE OF THIS LOGIC. SO REMOVING IT.
             ...
             substate_cv_forces = alphaprob * evdwl * substate_probability_partial[j][jsubstate]  * compute_proximity_function_der(jtype_actual, distance) * (1/distance);

             //printf("i %d isubstate %d j %d jsubstate %d substate_cv_forces x %f y %f z %f\n", tag[i], isubstate, tag[j], jsubstate, substate_cv_forces * delx, substate_cv_forces * dely, substate_cv_forces * delz);

             f[i][0] += substate_cv_forces * delx;
             f[i][1] += substate_cv_forces * dely;
             f[i][2] += substate_cv_forces * delz;
  ...
  // substate_cv_backforce on local atoms need to be accumulated from neighboring procs using reverse comm
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (n_states_per_type[itype_actual] > 1) {
      ...
      for (isubstate = 0; isubstate < n_states_per_type[itype_actual] ; isubstate++) { // SD removed -1
        ...
        for (jj = 0; jj < jnum; jj++) {
          ...
          rsq = delx * delx + dely * dely + delz * delz;
          jtype = type[j];
          jtype_actual = actual_types_from_state[jtype]; // SD
          ...
          if (rsq < cutsq[itype][jtype]) {
            distance = sqrt(rsq);
            fpair = cv_force * compute_proximity_function_der(itype_actual, distance); // SD
            fpair /= distance; // SD

            //printf("i %d isubstate %d cv_force %f x %f y %f z %f\n",tag[i], isubstate, cv_force, fpair*delx, fpair*dely, fpair*delz);
            ...
            ev_tally_full(i, 0.0, 0.0, pair_force, delx, dely, delz); // SD
          }
        } // end of jj

      } // end of isubstate
    } // end if nstates_per_type[itype_actual] > 1
  } // end of ii
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    //printf("sf forces: i %d %f %f %f\n", tag[i], substate_cv_backforce[i][0], substate_cv_backforce[i][1], substate_cv_backforce[i][2]);
    f[i][0] += substate_cv_backforce[i][0];
    f[i][1] += substate_cv_backforce[i][1];
    f[i][2] += substate_cv_backforce[i][2];
    //printf("ff after: i %d %f %f %f\n", tag[i], f[i][0], f[i][1], f[i][2]);
    //printf("-----------------------\n");
  }
...
void PairTableUCGDNA::settings(int narg, char **arg)
{
...
void PairTableUCGDNA::read_state_settings(const char *file) {
  char *eof;
  char line[MAXLINE];
  ...
  for (int i = 1; i <= n_actual_types; i++) {
    ...
    printf("probability_scaling_factor[0]: %lg \t probability_scaling_factor[1]: %lg \n", probability_scaling_factor[0], probability_scaling_factor[1]);
...
void PairTableUCGDNA::coeff(int narg, char **arg)
{
...
void PairTableUCGDNA::init_style()
{
...
double PairTableUCGDNA::init_one(int i, int j)
{
...
void PairTableUCGDNA::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTableUCGDNA::bcast_table(Table *tb)
{
...
void PairTableUCGDNA::spline_table(Table *tb)
{
...
void PairTableUCGDNA::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTableUCGDNA::compute_table(Table *tb)
{
...
void PairTableUCGDNA::null_table(Table *tb)
{
...
void PairTableUCGDNA::free_table(Table *tb)
{
...
void PairTableUCGDNA::spline(double *x, double *y, int n,
...
double PairTableUCGDNA::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTableUCGDNA::write_restart(FILE *fp)
{
...
void PairTableUCGDNA::read_restart(FILE *fp)
{
...
void PairTableUCGDNA::write_restart_settings(FILE *fp)
{
...
void PairTableUCGDNA::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTableUCGDNA::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTableUCGDNA::extract(const char *str, int &dim)
{
